...
if x(1)+x(2)+x(3)==0
    ...
    A=(2*x(1).*x(2)+x(1).*x(2)+x(2).*x(3)+0.5*x(2).^2);
     dx=zeros(3,1);
   dx(1)= beta*(x(1).^2+x(1).*x(2)+0.25*x(2).^2)./(x(1)+x(2)+x(3))-...
       ...(mu_M+u)*x(1)-D_M*(x(1).^2+psi1*x(1).*x(2)+psi2*x(1).*x(3));
   dx(2) = beta*A./(x(1)+x(2)+x(3))-(mu_M+u2)*x(2)...
   ...-D_M*(x(2).^2+(1-psi1)*x(1).*x(2)+psi3*x(2).*x(3));
   dx(3) = beta*(x(3).^2+x(3).*x(2)+0.25*x(2).^2)./(x(1)+x(2)+x(3))...
   ...-mu_M*x(3)-D_M*(x(3).^2+(1-psi2)*x(1).*x(3)+(1-psi3)*x(3).*x(2));
